...
const APP_NAME = "booking" as const;
const DISPATCH_URL = process.env.APP_URL
  ? process.env.APP_URL.replace(/\/booking.*$/, "")
  : "https://xdr-hub.netlify.app";

// Enforces per-app access grants for the Booking app.
...
export default defineEventHandler(async (event) => {
  ...
  return {
    error: `You don't have access to Booking. Visit ${DISPATCH_URL} to request access.`,
  };
});
