...
export default defineAction({
  ...
  schema: z.object({
    ...
    autoEnrollHubspotWorkflow: z
      .boolean()
      .default(true)
      .describe("When on, each reconcile tick creates a minimal HubSpot contact for each newly-synced CommonRoom/Prospector-leg contact and enrolls it in each persona's matching HubSpot workflow (resolved by name) -- the workflow itself handles everything past enrollment"),
  }),
  requiresAuth: true,
  http: { method: "POST" },
  run: async (
